...
func renderScanReportTable(report *schema.ScanReport, w io.Writer) error {
	...
	fmt.Fprintf(w, "agentmoat scan: %d workloads scanned\n", s.Total)
	fmt.Fprintf(w, "  compatible: %d   review: %d   incompatible: %d\n\n",
	...
	fmt.Fprintln(tw, "NAMESPACE\tKIND\tNAME\tVERDICT\tREASONS\tRECOMMENDATION")
...
func renderMigrationPlanTable(plan *schema.MigrationPlan, w io.Writer) error {
	s := plan.Spec.Summary
	fmt.Fprintf(w, "agentmoat plan: %d steps total (included: %d, excluded: %d)\n",
	...
	if len(plan.Spec.Steps) == 0 {
		fmt.Fprintln(w, "(no included steps)")
	} else {
		tw := tabwriter.NewWriter(w, 0, 2, 2, ' ', 0)
		fmt.Fprintln(tw, "#\tNAMESPACE\tKIND\tNAME\tRISK\tWAIT-FOR\tNOTES")
	...
	if len(plan.Spec.Excluded) > 0 {
		fmt.Fprintln(w, "")
		fmt.Fprintln(w, "Excluded workloads:")
		tw := tabwriter.NewWriter(w, 0, 2, 2, ' ', 0)
		fmt.Fprintln(tw, "NAMESPACE\tKIND\tNAME\tCOMPATIBILITY\tREASON")
...
func renderApplyResultTable(res *schema.ApplyResult, w io.Writer, action string) error {
	s := res.Spec.Summary
	fmt.Fprintf(w, "agentmoat %s: %d steps total\n", action, s.Total)
	fmt.Fprintf(w, "  applied: %d   already-applied: %d   skipped: %d   failed: %d\n",
		s.Applied, s.AlreadyApplied, s.Skipped, s.Failed)
	fmt.Fprintf(w, "  plan-hash: %s   dry-run: %v\n\n",
	...
	tw := tabwriter.NewWriter(w, 0, 2, 2, ' ', 0)
	defer tw.Flush()
	fmt.Fprintln(tw, "#\tNAMESPACE\tKIND\tNAME\tSTATUS\tNOTE")
